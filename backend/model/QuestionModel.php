...
class QuestionModel
{
    ...
    public function getList($params = [])
    {
        ...
        // 获取列表
        $sql = "SELECT q.*, c.name as chapter_name 
                FROM {$this->table} q 
        ...
        $list = $this->db->fetchAll($sql, array_merge($whereParams, [$pageSize, $offset]));
    ...
    {
        $questions = $this->db->fetchAll(
            "SELECT id FROM {$this->table} WHERE subject = ? AND status = 1 ORDER BY RAND() LIMIT ?",
            [$subject, $count]
        );
    ...
    public function getAdminList($params = [])
    {
        ...
        // 获取总数
        $total = $this->db->fetchColumn(
            "SELECT COUNT(*) FROM {$this->table} q {$whereSql}",
            $whereParams
        );
